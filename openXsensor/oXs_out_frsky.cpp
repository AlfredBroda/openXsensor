...
extern uint8_t volatile sendStatus ;
  ...
  #if ( defined(ARDUINO_MEASURES_VOLTAGES) && (ARDUINO_MEASURES_VOLTAGES == YES) ) || ( defined(AN_ADS1115_IS_CONNECTED) && (AN_ADS1115_IS_CONNECTED == YES) && defined(ADS_MEASURE) ) && defined(A3_SOURCE) && ( A3_SOURCE == CELLS_MIN )
    struct ONE_MEASUREMENT cells_min ;
  #endif
#endif
...
// pointer to A3
    ...
    idToReply |= 0x04 ;
#elif defined(A3_SOURCE)  && (defined(ARDUINO_MEASURES_VOLTAGES) || (defined(AN_ADS1115_IS_CONNECTED) && (AN_ADS1115_IS_CONNECTED == YES ))) && (A3_SOURCE == CELLS_MIN)
    p_measurements[8] =  &cells_min;
    idToReply |= 0x04 ;
#else
...
#if ( defined(ARDUINO_MEASURES_VOLTAGES) && (ARDUINO_MEASURES_VOLTAGES == YES) ) || ( defined(AN_ADS1115_IS_CONNECTED) && (AN_ADS1115_IS_CONNECTED == YES) && defined(ADS_MEASURE) ) && defined(A3_SOURCE) && ( A3_SOURCE == CELLS_MIN )
  if ( (!cells_min.available) && ( oXs_Voltage.voltageData.mVoltCellMin.available ) ){
    cells_min.value = oXs_Voltage.voltageData.mVoltCellMin.value / 10 ;  // voltage in mv is divided by 10 because SPORT expect it (volt * 100)
    cells_min.available = true ;
    oXs_Voltage.voltageData.mVoltCellMin.available = false ;
  }
#endif
