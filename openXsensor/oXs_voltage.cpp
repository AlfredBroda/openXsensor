...
//            voltageData.mVolt[cntVolt].value = (1 + cntVolt) * 3000 + cntVolt * (millis() & 0xFF)  ; // this is just to test the cell calculation ; !!!!!!!!!!!to be removed
        ...
        for (uint8_t cellIndex = 0; cellIndex < NUMBEROFCELLS ; cellIndex++) {
          ...
          voltageData.mVoltCell[cellIndex].value  = mVoltOneCell ;
          voltageData.mVoltCell[cellIndex].available = true ;
        }
        voltageData.mVoltCellMin.value = 0 ;
        voltageData.mVoltCellTot.value = 0 ;
        for (uint8_t cellIndex = 0; cellIndex < voltageData.maxNumberOfCells ; cellIndex++) {
          if (voltageData.mVoltCell[cellIndex].value == 0 ) {
            break ;
          } else {
            if ( (voltageData.mVoltCellMin.value == 0) || ( voltageData.mVoltCellMin.value > voltageData.mVoltCell[cellIndex].value ) ){
              voltageData.mVoltCellMin = voltageData.mVoltCell[cellIndex] ;
            }
              voltageData.mVoltCellTot = voltageData.mVolt[cellIndex] ;
          }
        }
        if ( voltageData.mVoltCellMin.value > 0 ) {
          voltageData.mVoltCellMin.available = true ;
        }
        voltageData.mVoltCellTot.available = true ;
